...
export class GameEngine{
  ...
  submit({result,bonus=0,weakness,questionType='general',target='',reducedBonus=false}={}){if(!this.active||this.locked)return false;this.locked=true;if(result==='near'){this.onRender({phase:'answer',near:true});return true}const ok=result==='correct';recordAttempt({game:this.game,level:this.level,questionType,target,correct:ok});if(ok){this.combo++;this.maxCombo=Math.max(this.maxCombo,this.combo);const points=Math.round((reducedBonus?75:100)*multiplier(this.combo))+bonus;this.score+=points;this.onRender({phase:'answer',ok:true,points,combo:this.combo});return true}this.wrong++;this.combo=0;this.lastWeakness=weakness||this.lastWeakness;this.onRender({phase:'answer',ok:false});return true;}
