...
export class GameEngine{
  ...
  answer(ok,{near=false,bonus=0,weakness,questionType='general',target=''}={}){if(!this.active||this.locked)return false;this.locked=true;if(!near)recordAttempt({game:this.game,level:this.level,questionType,target,correct:ok});if(ok){if(!near){this.combo++;this.maxCombo=Math.max(this.maxCombo,this.combo);this.score+=Math.round(100*multiplier(this.combo))+bonus;}}else{this.wrong++;this.combo=0;this.lastWeakness=weakness||this.lastWeakness;}this.onRender({phase:'answer',ok,near});return true;}
